    ...
    <button
      ...
      className={`p-3 rounded-xl transition-all duration-300 flex items-center justify-center ${
        copied
          ...
          : 'bg-[var(--brand)] text-[var(--text-inverse)] hover:brightness-110'
      }`}
    >
...
export default function Uploader({
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-4">
          {files.length === 1 && files[0].type.startsWith('image/') ? (
            <div 
              className="w-14 h-14 rounded-2xl bg-[var(--bg-elevated)] flex items-center justify-center text-[var(--text-secondary)] overflow-hidden shadow-inner border border-[var(--border-subtle)]"
              style={{
            ...
            <div className="w-14 h-14 rounded-2xl bg-[var(--bg-elevated)] flex items-center justify-center text-[var(--text-secondary)] overflow-hidden shadow-inner border border-[var(--border-subtle)] relative">
              <video 
          ...
          <div className="flex-1 overflow-hidden z-10">
            <h3 className="text-primary font-bold truncate text-lg">
              {files[0].name}{' '}
        ...
        <div className="mt-8 flex items-center gap-2 text-sm font-medium">
          {activeDownloaders > 0 ? (
            <>
              <span className="w-2.5 h-2.5 rounded-full bg-[var(--brand)] animate-pulse shadow-[0_0_8px_var(--brand-soft)]"></span>
            ...
            <>
              ...
              <span className="text-muted">Waiting for Peer...</span>
            </>
          )}
        </div>
        <div className="mt-4 grid grid-cols-3 gap-2">
          <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] px-3 py-2">
            <div className="text-[11px] text-muted uppercase tracking-wider">Peers</div>
            <div className="text-primary font-semibold text-sm mt-1">
              {connections.length || 0}
            </div>
          </div>
          <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] px-3 py-2">
            <div className="text-[11px] text-muted uppercase tracking-wider">Done</div>
            <div className="text-primary font-semibold text-sm mt-1">
              {completedPeers}
            </div>
          </div>
          <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] px-3 py-2">
            <div className="text-[11px] text-muted uppercase tracking-wider">Progress</div>
            <div className="text-primary font-semibold text-sm mt-1">
        ...
        {activeDownloaders > 0 && (
          <div className="absolute bottom-0 left-0 h-1.5 bg-[var(--bg-elevated)] w-full">
            <div
      ...
      >
        <div className="flex-1 bg-[var(--bg-app)] rounded-2xl px-5 py-4 truncate font-mono text-secondary text-sm border border-[var(--border-subtle)] shadow-inner">
          {shortURL}
        </div>
